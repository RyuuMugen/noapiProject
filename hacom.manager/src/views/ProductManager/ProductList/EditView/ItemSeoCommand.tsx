...
import { getDataMegaMenu, modifyMegaMenuGet } from "../../../../api/ApiMegaMenu";
...
const ItemSeoCommand = ({
  ...
  useEffect(() => {
    const dataOptions = dataItemSeo?.map((item: any) => {
      return {
  ...
  useEffect(() => {
    if (dataItemSeo[0]) {
      const getDataDataCategoryOption = async () => {
        ...
        let callapi = await modifyMegaMenuGet(
          urlCreate
        );
  ...
  useEffect(() => {
    if (dataItemSeoCommand) {
      setSelectedValues(dataItemSeoCommand);
    }
  }, [dataItemSeoCommand]);
...
type ItemCategoryCommandProps = {
  ...
  dataItemSeoCommand: TblItemSeoCommand[] | null;
};
