...
import {
  tblProductAdsCategory,
  tblProductAdsCategoryList,
...
const ProductAdsCategory = () => {
  const columns: Array<EuiBasicTableColumn<any>> = [
    ...
    },
    {
      ...
      render: (itemName: string) => (
        <EuiLink
          target="_blank"
          onClick={(e: any) => {
            const item = datas.find((x) => x.categoryName === itemName);
    ...
    },
    {
      ...
      truncateText: true,
      width: "20%",
      render: (category: tblProductAdsCategory) => (
        <EuiLink
          target="_blank"
          onClick={() =>
            navigate("/product-ads", { state: { id: category.id } })
          }
        >
          Xem
        </EuiLink>
    ...
    },
    {
      ...
      truncateText: true,
      width: "20%",
      render: (feed: string) => <EuiLink target="_blank">{feed}</EuiLink>,
    },
    {
      field: "googleFeed",
      name: "Google feed",
  ...
  const [datas, setDatas] = useState<tblProductAdsCategory[]>([]);
  ...
  const openModal = () =>
    modals.openConfirmModal({
      title: (
        <>
          <div color="white">
            <Title order={5}>Thêm mới danh mục Product Ads</Title>
          </div>
        </>
      ),
      children: (
  ...
  function editItem(id: number) {
    modals.openConfirmModal({
      title: (
        <>
          <Title order={5}>Chỉnh sửa danh mục Product Ads!</Title>
        </>
      ),
  ...
  const fetchDataProductAdsCategory = async (
    index: number,
    size?: number,
    keyWord?: string,
    inActive?: boolean
    ...
    try {
      let urlSearch = `${API_ROUTE.GET_LIST_PRODUCT_ADS_CATEGORY}?Skip=${
        index * (size ?? 0)
      }&Take=${size}`;
      ...
      let callapi = await repository.get<
        MessageResponse<tblProductAdsCategory[]>
      >(urlSearch);
  ...
  useEffect(() => {
    const fetchData = async () => {
      await fetchDataProductAdsCategory(
        pagination.pageIndex,
        pagination.pageSize,
        paramSearch?.keyWord,
        paramSearch?.inActive
...
export default ProductAdsCategory;
