...
const TradeInOrderNew = () => {
  const columns: Array<EuiBasicTableColumn<any>> = [
    ...
    {
      field: "",
      name: "Sản phẩm muốn đổi lên",
      sortable: true,
      truncateText: true,
      render: (online: TblTradeInOrder) => (
        <Box>
          <List>
            <List.Item>Tên: {online.itemNameUpdate}</List.Item>
            <List.Item>
              Giá tại thời điểm đặt:{" "}
              <NumberFormatter
                value={online.itemPriceUpdate || 0}
                thousandSeparator
                suffix=" VND"
              />
            </List.Item>
            <List.Item>
              Sản phẩm của web:{" "}
              <EuiLink href={online.linkIndexUpdate || ""}>
                {online.itemUpgradeCode}
              </EuiLink>
            </List.Item>
          </List>
        </Box>
...
export default TradeInOrderNew;
