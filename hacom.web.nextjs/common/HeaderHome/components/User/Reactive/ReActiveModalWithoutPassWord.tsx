...
const ReActiveModalWithoutPassword: React.FC<ReActiveModalProps> = ({
  ...
  const handleSendActivationEmail = async () => {
    ...
    try {
      ...
      if (response.httpStatusCode && response.success) {
        setError(
          "Gửi mã kích hoạt thành công , vui lòng kiểm tra tin nhắn hoặc email của bạn"
        );
      } else {
  ...
  const handleSendActiveCode = async () => {
    try {
      ...
      if (response.httpStatusCode && response.success) {
        setError(
          "Gửi mã kích hoạt thành công , vui lòng kiểm tra hộp thư máy hoặc email"
        );
      } else {
        setError("Gửi mã kích hoạt thất bại vui lòng thử lại sau");
      }
        ...
        <div className="modal">
          <div className="modal-content">
            <div className={style.modalContent}>
              <h4>Bạn hãy nhập số điện thoại hoặc email dùng để đăng ký</h4>
              ...
              <div className={style.comfirmBox}>
                ...
                >
                  {isButtonDisabled ? `${countdown} giây` : "Gửi mã/Link"}
                </button>
              </div>
            </div>
            <div className={style.modalContent}>
              <h4 style={{ textAlign: "center" }}>
                Nhập mã kích hoạt vào đây (
                <span style={{ color: "red" }}>
                  nếu bạn sử dụng số điện thoại
                </span>
                )<br></br>
                Nếu sử dụng email vui lòng kiểm tra hộp thư đến
              </h4>
            ...
            <Center m={"20px 0px"}>
              <Flex gap={"5px"}>
                <Text>
                  Chưa nhận được mã kích hoạt hoặc link kích hoạt? Nhấn lại vào
                  nút
                </Text>
                <span className={style.signUp}>GỬI MÃ/LINK sau 60 giây</span>
              </Flex>
            </Center>
          </div>
        </div>
