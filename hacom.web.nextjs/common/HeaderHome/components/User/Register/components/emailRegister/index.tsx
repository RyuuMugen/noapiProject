...
import { createCustomer, createCustomerWithOutToken } from "@/api/apiCustomer";
...
const RegisterModal = () => {
  ...
  const handleCreateAccount = async (dataSubmit: Register) => {
    await AuthService.registerEmail(dataSubmit);
    await createCustomerWithOutToken({
      ...customer,
      userName: dataSubmit.username,
      customerName: dataSubmit.fullName,
      email: dataSubmit.username,
  ...
  const floating3 = focused3 || form.values.username.length > 0 || undefined;
  ...
  return (
    ...
    >
      ...
      <Container>
        ...
        <button type="submit" className={captcha ? style.button: style.disableButton} disabled={!captcha}>
          Tạo tài khoản
        </button>
