...
const CustomerAppointment = () => {
  const columns: Array<EuiBasicTableColumn<any>> = [
    ...
    },
    {
      ...
      width: "13%",
      truncateText: true,
      render: (online: TblCustomerAppointment) => (
        <div>
          <Text fw={500}>{online.fullName}</Text>
          <p>{online.email}</p>
          <p>Sđt: {online.mobile}</p>
        </div>
      ),
    },
    {
      ...
      sortable: true,
      truncateText: true,
      render: (productName: string, collection: TblCustomerAppointment) => (
        <EuiLink
          target="_blank"
          onClick={(e: any) => {
  ...
  const [datas, setDatas] = useState<TblCustomerAppointment[]>([]);
  ...
  function openModalView(id: number) {
    modals.openConfirmModal({
      title: (
        <>
          <Title order={5}>Xem chi tiết đơn!</Title>
        </>
      ),
      children: <ModalView id={id} />,
      confirmProps: { display: "none" },
      cancelProps: { display: "none" },
    });
  }
  ...
  const fetchDataCustomerAppointment = async (
    index: number,
    size?: number,
    keyWord?: string,
    inActive?: boolean
    ...
    try {
      let urlSearch = `${API_ROUTE.GET_LIST_CUSTOMER_APPOINTMENT}?Skip=${
        index * (size ?? 0)
      }&Take=${60}`;
  ...
  useEffect(() => {
    const fetchData = async () => {
      await fetchDataCustomerAppointment(
        pagination.pageIndex,
        pagination.pageSize,
        paramSearch?.keyWord,
        paramSearch?.inActive
