...
const Appointment = () => {
  const columns: Array<EuiBasicTableColumn<any>> = [
    ...
    },
    {
      ...
      width: "15%",
      truncateText: true,
      render: (online: TblAppointment) => (
        <div>
          <Text fw={500}>{online.customerName}</Text>
          <p>Sđt: {online.phoneNumber}</p>
        </div>
      ),
    },
    {
      ...
      width: "12%",
      truncateText: true,
      render: (deviceType: string, collection: TblAppointment) => (
        <EuiLink
          target="_blank"
          onClick={(e: any) => {
  ...
  const [datas, setDatas] = useState<TblAppointment[]>([]);
  ...
  const fetchDataAppointment = async (
    ...
    try {
      let urlSearch = `${API_ROUTE.GET_LIST_APPOINTMENT}?Skip=${
        index * (size ?? 0)
      }&Take=${60}`;
