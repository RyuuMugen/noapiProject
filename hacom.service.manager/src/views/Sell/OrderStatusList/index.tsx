...
const OrderStatus = () => {
    ...
    },
    {
      ...
      render: (status: string) => (
        <EuiLink
          target="_blank"
          onClick={(e: any) => {
            const id = datas.find((x) => x.status === status);
  ...
  const openModal = () =>
    modals.openConfirmModal({
      title: (
        <>
          <div color="white">
            <Title order={5}>Thêm trạng thái đơn hàng mới</Title>
          </div>
        </>
      ),
      children: (
  ...
  function editItem(id: number) {
    modals.openConfirmModal({
      title: (
        <>
          <Title order={5}>Cập nhật lại trạng thái đơn hàng!</Title>
        </>
      ),
  ...
  const fetchDataOrderStatus = async (
    ...
    try {
      let urlSearch = `${API_ROUTE.GET_LIST_ORDER_STATUS}?Skip=${
        index * (size ?? 0)
      }&Take=${size}`;
      ...
      if (
        ...
        const lists = callapi?.data.lists;
        setDatas(lists);
        setPagination({
          ...pagination,
