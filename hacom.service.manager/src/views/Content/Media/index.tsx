...
import { TblMedia } from "../../../model/TblMedia";
...
const Media = () => {
  const columns: Array<EuiBasicTableColumn<any>> = [
    ...
    },
    {
      ...
      render: (itemName: string) => (
        <EuiLink
          target="_blank"
          onClick={(e: any) => {
            const item = datas.find((x) => x.mediaName === itemName);
            if (item !== undefined && item.mediaId > 0)
              handleEditData(item?.mediaId);
          }}
        >
          {itemName}
        </EuiLink>
    ...
    },
    {
      ...
      truncateText: true,
      width: "20%",
      render: (linkFile: string) => (
        <EuiLink target="_blank" href={linkFile}>
          Xem trang bài viết
        </EuiLink>
      ),
    },
  ...
  const [datas, setDatas] = useState<TblMedia[]>([]);
  ...
  const openModal = () =>
    modals.openConfirmModal({
      title: (
        <>
          <div color="white">
            <Title order={5}>Thêm mới media</Title>
          </div>
        </>
      ),
      children: (
  ...
  const openModalEdit = () => {
    if (
      ...
      editItem(isSelectedItem[0].mediaId);
    }
  };
  ...
  const openModalDelete = () => {
    if (isSelectedItem && isSelectedItem.length < 1)
      ...
      deleteItem(ids.map((item) => item?.mediaId));
    }
  };
  ...
  function editItem(id: number) {
    modals.openConfirmModal({
      title: (
        <>
          <Title order={5}>Chỉnh sửa media!</Title>
        </>
      ),
  ...
  const fetchDataMedia = async (
    ...
    try {
      let urlSearch = `${API_ROUTE.GET_LIST_MEDIA}?Skip=${
        index * (size ?? 0)
      }&Take=${size}`;
