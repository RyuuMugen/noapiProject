...
const CreateView = ({ id, onSearch }: { id: number; onSearch: any }) => {
  ...
  const handleCreateCollection = async (dataSubmit: TblCollection) => {
    open();
    await editCollection(dataSubmit);
    close();
    modals.closeAll();
    onSearch();
  };
      ...
      >
        ...
        <Group grow wrap="nowrap" mt="lg" gap={"lg"}>
          <Select
            ...
            clearable
            disabled={form.values.tblComboSetCollectionModels.length > 0}
