...
interface ItemYoutobe {
  linkYoutube: string;
  description: string;
}

const ItemYoutobeCommand = ({ onAddYoutobe }: ItemYoutobeProps) => {
  const [dataYoutobe, setDataYoutobe] = useState<ItemYoutobe[]>([]); // Lưu giữ giá trị của TextInput cho từng Group
      ...
      {dataYoutobe.map((_, index) => (
        <Group key={index} grow wrap="nowrap" mt="xs" gap={"lg"}>
          <TextInput
            ...
            mt="md"
            type="text"
            value={dataYoutobe[index]?.linkYoutube}
            onChange={(e) =>
          ...
          <TextInput
            ...
            mt="md"
            type="text"
            value={dataYoutobe[index]?.description}
            onChange={(e) =>
...
export default ItemYoutobeCommand;
